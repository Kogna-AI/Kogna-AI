...
const getPageQuickActions = (view: string) => {
  ...
  return {
    base: baseActions,
    specific:
      pageSpecificActions[view as keyof typeof pageSpecificActions] || [],
  };
};
...
export function KogniiAssistant({
  ...
  const [_sessionActive, _setSessionActive] = useState(strategySessionMode);
  const [_sessionStep, _setSessionStep] = useState(0);
  const [_currentContext, _setCurrentContext] = useState(activeView);
  const [_demoMode, _setDemoMode] = useState(true);
  ...
  }, [activeView, conversationMode, strategySessionMode]);
  ...
  const handleSendMessage = async () => {
    ...
    if (!sessionId) {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          type: "assistant",
          content:
            "There was an error connecting to the chat session. Please close and reopen the assistant.",
          timestamp: new Date(),
        },
      ]);
      return;
    ...
    try {
      ...
      const aiMessage: Message = {
        ...
        timestamp: new Date(),
        suggestions: [
          // You can have the AI generate these later
          "Tell me more",
  ...
  const generateAIResponse = (input: string): string => {
    ...
    return (
      responses[input as keyof typeof responses] ||
      `I understand you're asking about "${input}". Based on the current context in ${pageContexts[activeView as keyof typeof pageContexts]?.name || "this section"}, I can provide detailed insights and actionable recommendations.\n\nLet me analyze your specific situation and provide tailored advice. What aspect would you like me to focus on first?`
    );
  };
  ...
  const handleQuickAction = (action: string) => {
    ...
    const response =
      actionResponses[action as keyof typeof actionResponses] ||
  ...
  const _formatChatHistory = (msgs: Message[]) => {
    ...
    return (
      <div className="w-96 h-full bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-30">
          ...
          <div
            className="absolute top-1/4 left-1/4 w-24 h-24 bg-gradient-to-br from-cyan-400 to-blue-600 rounded-full blur-3xl animate-bounce"
            style={{ animationDuration: "3s" }}
          ></div>
          <div
            className="absolute bottom-1/4 right-1/4 w-20 h-20 bg-gradient-to-br from-purple-400 to-pink-600 rounded-full blur-3xl animate-pulse"
            style={{ animationDuration: "2s" }}
          ></div>
          <div
            className="absolute top-1/2 right-1/3 w-16 h-16 bg-gradient-to-br from-indigo-400 to-purple-600 rounded-full blur-3xl animate-bounce"
          ...
          <div className="bg-white/5 backdrop-blur-lg rounded-xl border border-white/10 h-full p-4 flex flex-col">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-white/60">
                  Immersive AI strategic planning
                </p>
              </div>
            </div>
            ...
            <div className="mb-4 flex items-center justify-center gap-2">
              <Button
                ...
                disabled={
                  isAutoPlaying ||
                  conversationStep >= conversationScenario.length - 1
                }
                size="sm"
                    ...
                    <div className="bg-green-500/10 backdrop-blur border border-green-400/20 rounded-lg p-3 animate-in slide-in-from-bottom duration-700">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-white/80">
                            Move Sarah to mobile team (85% match)
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-blue-400 rounded-full"></div>
                          <span className="text-xs text-white/80">
                            Hire contractor for analytics (temp solution)
                          </span>
                        </div>
                      </div>
                    </div>
