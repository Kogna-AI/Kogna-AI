...
export const api = {
  ...
  updateOrganization: async (orgId: number, data: {
    name: string;
    industry?: string;
    team_due?: number;
    team?: string;
  ...
  getDashboard: async (orgId: number) => {
    const response = await fetch(`${API_BASE_URL}/organizations/${orgId}/dashboard`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  updateObjective: async (objId: number, data: {
    title?: string;
    progress?: number;
    status?: string;
    team_responsible?: string;
  ...
  achieveMilestone: async (milestoneId: number) => {
    const response = await fetch(`${API_BASE_URL}/milestones/${milestoneId}/achieve`, {
      method: 'PUT',
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  ...
  listMetrics: async (orgId: number, filters?: { name?: string; limit?: number }) => {
  ...
  getMetricTrends: async (orgId: number) => {
    const response = await fetch(`${API_BASE_URL}/organizations/${orgId}/metrics/trends`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  listInsights: async (orgId: number, filters?: {
    category?: string;
    level?: string;
    status?: string;
  }) => {
  ...
  archiveInsight: async (insightId: number) => {
    const response = await fetch(`${API_BASE_URL}/insights/${insightId}/archive`, {
      method: 'PUT',
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  ...
  listRecommendations: async (orgId: number, filters?: {
    status?: string;
    created_for?: number;
    ...
    if (filters?.created_for) params.append('created_for', filters.created_for.toString());
  ...
  getPendingRecommendations: async (orgId: number) => {
    const response = await fetch(`${API_BASE_URL}/organizations/${orgId}/recommendations/pending`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  getHighPriorityRecommendations: async (orgId: number) => {
    const response = await fetch(`${API_BASE_URL}/organizations/${orgId}/recommendations/high-priority`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  updateRecommendationStatus: async (recId: number, status: 'pending' | 'acted' | 'dismissed') => {
    const response = await fetch(`${API_BASE_URL}/recommendations/${recId}/status?new_status=${status}`, {
      method: 'PUT',
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  ...
  updateRecommendation: async (recId: number, data: {
    organization_id: number;
    title: string;
    recommendation: string;
    confidence: number;
  ...
  assignRecommendation: async (recId: number, userId: number) => {
    const response = await fetch(`${API_BASE_URL}/recommendations/${recId}/assign?user_id=${userId}`, {
      method: 'POST',
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  ...
  addRecommendationReason: async (recId: number, data: {
    recommendation_id: number;
    reason: string;
    evidence_datasets_id?: any;
  }) => {
    const response = await fetch(`${API_BASE_URL}/recommendations/${recId}/reasons`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify(data),
    });
  ...
  getRecommendationStats: async (orgId: number) => {
    const response = await fetch(`${API_BASE_URL}/organizations/${orgId}/recommendations/stats`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  getRecommendationActions: async (recId: number) => {
    const response = await fetch(`${API_BASE_URL}/recommendations/${recId}/actions`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  getUserActions: async (userId: number, limit: number = 50) => {
    const response = await fetch(`${API_BASE_URL}/users/${userId}/actions?limit=${limit}`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
